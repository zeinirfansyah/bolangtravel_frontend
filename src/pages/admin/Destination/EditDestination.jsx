...
export const EditDestination = () => {
  ...
  const getDestination = async () => {
    try {
      ...
      setDestinationContact(result?.destination_contact);
      setExpiryDate(result?.expiry_date);
      setIsAvailable(result?.is_available);
      setOpenHour(result?.open_hour);

      setThumbnailUrl(DOMAIN_URL + result?.thumbnail);
    } catch (error) {
      console.log(error);
    }
  };
      ...
      <div className="px-4">
        <div className="flex flex-col gap-5 min-h-[90vh] py-4">
          <div className="flex gap-2 bg-white border-b-2 p-4 mb-4">
            ...
            <p>/</p>
            <p className="text-slate-700 font-bold">Edit Destination</p>
          </div>
